    ...
    <div class="header-content container">
      ...
      <div class="header-actions">
        ...
        <template v-if="authStore.isAuthenticated">
          <!-- Avatar con dropdown de opciones -->
          <div class="user-menu-wrap" ref="userMenuRef">
            ...
            >
              ...
              <span class="material-symbols-outlined chevron" :class="{ rotated: userMenuOpen }">expand_more</span>
            </button>
            ...
            <Transition name="dropdown">
              <div v-if="userMenuOpen" class="user-dropdown" @click.stop>
                ...
                <div class="dropdown-header">
                  <img v-if="authStore.user?.avatar" :src="authStore.user.avatar" class="dd-avatar-img" alt="Avatar" />
                  <span v-else class="dd-avatar">{{ authStore.initials }}</span>
                  <div class="dd-header-info">
                    <span class="dd-name">{{ authStore.displayName }}</span>
                    <span class="dd-email">{{ authStore.user?.email }}</span>
                  </div>
                </div>
                ...
                <!-- Opciones -->
                <button class="dd-item" @click="goTo(dashboardRoute)">
                  <span class="material-symbols-outlined">dashboard</span>
                  <span>Mi panel</span>
                </button>

                <button class="dd-item" @click="goTo(perfilRoute)">
                  <span class="material-symbols-outlined">manage_accounts</span>
                  <span>Configurar perfil</span>
                </button>

                <button v-if="authStore.user?.role === 'user'" class="dd-item" @click="goTo('/perfil?tab=pagos')">
                ...
                <button class="dd-item dd-item-help" @click="goTo('/contacto')">
                  <span class="material-symbols-outlined">help_outline</span>
                  <span>Necesito ayuda</span>
                </button>

                <button class="dd-item dd-item-logout" @click="handleLogout">
          ...
          <template v-if="authStore.isAuthenticated">
            <div class="mobile-user-row">
              ...
              <div class="mobile-user-actions">
                <button class="mobile-action-btn" @click="router.push(dashboardRoute); menuOpen = false">
                  <span class="material-symbols-outlined">dashboard</span>
                  Mi panel
                </button>
                <button class="mobile-action-btn" @click="router.push(perfilRoute); menuOpen = false">
                  <span class="material-symbols-outlined">manage_accounts</span>
                  Mi perfil
                </button>
                <button v-if="authStore.user?.role === 'user'" class="mobile-action-btn" @click="router.push('/perfil?tab=pagos'); menuOpen = false">
                  <span class="material-symbols-outlined">credit_card</span>
                  Métodos de pago
                </button>
                <button class="mobile-action-btn help" @click="router.push('/contacto'); menuOpen = false">
                  <span class="material-symbols-outlined">help_outline</span>
                  Necesito ayuda
                </button>
...
// Computed: ruta del dashboard según rol
...
// Computed: ruta de perfil dentro del dashboard
...
.user-dropdown {
  ...
  box-shadow: 0 8px 32px rgba(26,91,130,.16), 0 2px 8px rgba(26,91,130,.08);
  border: 1.5px solid rgba(197,140,242,.15);
  overflow: hidden;
  z-index: 999;
}
...
.dropdown-header {
  ...
  background: linear-gradient(135deg, rgba(197,140,242,.08), rgba(197,140,242,.03));
}
.dd-avatar-img {
  width: 2.4rem;
  height: 2.4rem;
...
.dd-item:hover {
  background: rgba(197,140,242,.08);
  color: #7c3aed;
}
.dd-item:hover .material-symbols-outlined { color: #c58cf2; }
...
.dd-item-help:hover { background: rgba(59,130,246,.07); color: #1d4ed8; }
.dd-item-help:hover .material-symbols-outlined { color: #3b82f6; }

.dd-item-logout {
  color: #e53e3e;
  margin-bottom: 0.2rem;
}
.dd-item-logout:hover { background: rgba(229,62,62,.08); }
.dd-item-logout .material-symbols-outlined { color: #e53e3e; }

/* Dropdown transition */
.dropdown-enter-active { transition: all 0.2s cubic-bezier(.34,1.56,.64,1); }
...
.mobile-action-btn {
  ...
  font-weight: 600;
  color: var(--text-blue);
  background: rgba(197,140,242,.07);
  border: 1px solid rgba(197,140,242,.2);
  border-radius: 5.5rem;
  padding: 0.6rem 1rem;
  cursor: pointer;
  transition: all 0.2s;
...
.mobile-action-btn:hover { background: rgba(197,140,242,.15); }
.mobile-action-btn .material-symbols-outlined { font-size: 1.1rem; color: #c58cf2; }
.mobile-action-btn.help { background: rgba(59,130,246,.07); border-color: rgba(59,130,246,.2); }
.mobile-action-btn.help .material-symbols-outlined { color: #3b82f6; }
</style>
